...
\]
are dual to each other, where $\sF$ is a locally free sheaf, $\sF^{\vee}$is the dual sheaf $\underline{\Hom}(\sF,\sO_{X})$, and $\somega=\Omega_{X/k}^{n}$ is the sheaf of $n$-differentials on $X$. Serre \cite{15} gives a proof of this same theorem by analytic methods for a compact complex analytic manifold $X$.

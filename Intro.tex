...
\[
\mathcal{D} = \mathbb{R}\mathcal{H}\mathit{om}(\,\cdot\,, R^{\bullet})
\]
treated here will be useful for our bootstrap operation (construction of $f^{!}$) later, because it interchanges $\otimes$ and $\mathcal{H}\mathit{om}$, $f^{!}$ and $\mathbb{L}f_{*}$, and commutes with $\mathbb{R}f_{*}$ (duality theorem).

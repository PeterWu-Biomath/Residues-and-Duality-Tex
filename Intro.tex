...
h^{0}(D)=h^{n}(K-D).
\]
This is also equivalent to a theorem of Serre \cite[FAC, \S 3, Thm.~3]{15} on the vanishing of the cohomology group $H^{1}(X, \sO_{X}(-m))$ for $m$ large and $\sL$ locally free. Using a related theorem \cite[FAC, \S 3, Thm.~3]{15}, Zariski shows how one can deduce on a non-singular projective variety the formula
\[
h^{i}(D)=h^{n-i}(K-D)
\]
for $0\leq i\leq n$. In terms of sheaves, this result corresponds to the fact that the $k$-vector spaces
\[
H^{i}(X, \sF) \quad\text{and}\quad H^{n-i}\bigl(X, \sF^{\vee}\otimes\somega\bigr)
\]
are dual to each other, where $\sF$ is a locally free sheaf, $\sF^{\vee}=\underline{\Hom}(\sF,\sO_{X})$, and $\somega=\Omega_{X/k}^{n}$ is the sheaf of $n$-differentials on $X$. Serre \cite{15} gives a proof of this same theorem by analytic methods for a compact complex analytic manifold $X$.
...
\begin{equation}\label{eq:1}
	H^{n}(X, \somega)\cong k
\end{equation}
where $\somega=\Omega_{X/k}^{n}$ is the sheaf of $n$-differentials. Combining this with the Yoneda pairing
\begin{equation}\label{eq:2}
	H^{i}(X, \sF)\times \Ext_{X}^{n-i}(\sF,\somega)\to H^{n}(X,\somega)
\end{equation}
we obtain a pairing
\begin{equation}\label{eq:3}
	H^{i}(X, \sF)\times \Ext_{X}^{n-i}(\sF,\somega)\to k
\end{equation}
which one shows easily to be a perfect pairing [SGA 62, exposé 12]. This generalizes the statements above, because for a locally free sheaf $\sF$,
\[
\Ext^{n-i}(\sF,\somega)=\Ext^{n-i}\bigl(\sO_{X},\sF^{\vee}\otimes\somega\bigr)=H^{n-i}\bigl(X,\sF^{\vee}\otimes\somega\bigr).
\]

Another way of looking at our duality pairing is as an isomorphism
\begin{equation}\label{eq:4}
\Ext_{X}^{n-i}(\sF,\somega)\cong \Hom_{k}\bigl(H^{i}(X,\sF),k\bigr).
\end{equation}
Since everything is linear over $k$, we may introduce a $k$-vector space $G$, and have an isomorphism
\begin{equation}\label{eq:5}
	\Ext_{X}^{n-i}\bigl(\sF,G\otimes_{k}\somega\bigr)\to \Hom_{k}\bigl(H^{i}(X,\sF),G\bigr).
\end{equation}
...
\begin{equation}\label{eq:7}
	\DerR\OHom_{X}\bigl(\sF,f^{!}\sG\bigr)\to \DerR\OHom_{Y}\bigl(\DerR f_{*}\sF,\DerR f_{*}\,f^{!}\sG\bigr),
\end{equation}
which, composed with the trace map \eqref{eq:6} gives us the duality morphism
\begin{equation}\label{eq:8}
	\DerR\OHom_{X}\bigl(\sF,f^{!}\sG\bigr)\to \DerR\OHom_{Y}\bigl(\DerR f_{*}\sF,\sG\bigr)
\end{equation}
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		f^{!}(\sG)=f^{*}(\sG)\otimes\somega,
		\]
		where $\somega=\Omega_{X/Y}^{n}$ is the sheaf of highest order differentials;
